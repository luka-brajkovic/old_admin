        ...
        if ($this->key_value !== false) {
            ...
            foreach ($fields as $key => $field_info) {
                if ($field_info['Field'] != $this->key_field) {
                    $SQL .= "`" . $field_info['Field'] . "` = '" .  mysql_real_escape_string($this->{$field_info['Field']}) . "', ";
                }
            }
            ...
            foreach ($fields as $key => $field_info) {
                if ($field_info['Field'] != $this->key_field) {
                    $SQL .= "`" . mysql_real_escape_string($field_info['Field']) . "`, ";
                }
            }
            $SQL = substr($SQL, 0, strlen($SQL) - 2);
            $SQL .= ") VALUES (";
            foreach ($fields as $key => $field_info) {
                if ($field_info['Field'] != $this->key_field) {
                    $SQL .= "'" . mysql_real_escape_string($this->{$field_info['Field']}) . "', ";
                }
            }
            $SQL = substr($SQL, 0, strlen($SQL) - 2);
            $SQL .= ")";
