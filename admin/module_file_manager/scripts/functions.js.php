...
dir = dir ? dir : new String('<?= str_replace("'", "\'", extGetParam($_SESSION, 'ext_' . $GLOBALS['file_mode'] . 'dir', '')) ?>');
var dirs = dir.split('/');
...
var re = /(?:<script([^>] * )? > )((\n | \r | .) * ?)(?: < \/script>)/ig;
    ...
    Ext.Msg.alert('<?= ext_Lang::err('error', true) . "','" . ext_Lang::err('miscselitems', true) ?>');
    return false;
    }
    ...
    id: "dialog",
            ...
            title: '<?= ext_Lang::msg('dialog_title', true) ?>'
    ...
    Ext.Ajax.request({ url: '<?= basename($GLOBALS['script_name']) ?>',
            ...
            msgbox = Ext.Msg.alert("<?= ext_Lang::err('error', true) ?>", "JSON Decode Error: " + e.message);
            msgbox.setIcon(Ext.MessageBox.ERROR);
            return false;
            }
            if (action == "edit") {
            ...
            msgbox = Ext.Msg.alert("<?= ext_Lang::err('error', true) ?>", "Received an empty response");
            msgbox.setIcon(Ext.MessageBox.ERROR);
            }
            }
    });
    ...
    Ext.Msg.confirm('<?= ext_Lang::msg('dellink', true) ?>?', String.format("<?= ext_Lang::err('miscdelitems', true) ?>", num), deleteFiles);
    break;
    case 'extract':
            Ext.Msg.confirm('<?= ext_Lang::msg('extractlink', true) ?>?', "<?= ext_Lang::msg('extract_warning', true) ?>", extractArchive);
    break;
    case 'download':
            document.location = '<?= basename($GLOBALS['script_name']) ?>?option=com_extplorer&action=download&item=' + encodeURIComponent(ext_itemgrid.getSelectionModel().getSelected().get('name')) + '&dir=' + encodeURIComponent(datastore.directory);
    break;
    }
    }
    ...
    Ext.get('x-box-mc-inner').setStyle('background-image', 'url("<?= _EXT_URL ?>/images/_accept.png")');
    ...
    if (success) {
    statusBar.setStatus({
    text: '<?= ext_Lang::msg('success', true) ?>: ' + msg,
            iconCls: 'success',
            clear: true
    });
    Ext.msgBoxSlider.msg('<?= ext_Lang::msg('success', true) ?>', msg);
    } else if (success != null) {
    statusBar.setStatus({
    text: '<?= ext_Lang::err('error', true) ?>: ' + msg,
            iconCls: 'error',
            clear: true
    });
    }
