    ...
    if ($proizvod->marketing_description != "") {
        ...
        <div id='opis' class="tabcontent <?php if($uso == '' && ($proizvod->technical_description == "" || $proizvod->warranty == "")){echo "actiTab";}; ?>">
            <div class="productPageSpec margin-vertical">
                ...
                <br>
                <p>* <?= $configSiteFirm; ?> nastoji da opis proizvoda bude što preciznija, ali ne možemo da garantujemo da su svi opisi kompletni i bez grešaka.<br>
    ...
    <div class="productPageSpec margin-vertical">
        ...
        if ($userData->resource_id) {
            if ($userData->title != "" && $nameCom == "") {
                ...
                $userDataMail = mysql_fetch_object($userDataMail);
            } else {
                $nameMess = $nameCom;
            }
            ...
            <div class="comentForm">
                <form action="<?php echo htmlspecialchars($REQUEST); ?>#pitanjeTab" method="post" class="clear">
                    ...
                    <p class="capcha<?= (in_array("capcha", $greske)) ? ' errorRed' : ""; ?>">
                        <span><?= $capcha1 . " <i style='line-height: 35px;' class='fa fa-plus plusImage'></i> " . $capcha2 . " = "; ?></span>
        ...
        <?php
        $quest = mysql_query("SELECT * FROM _content_komentari WHERE proizvod = '$proizvod->resource_id' AND odgovor != '' ORDER BY id DESC");
        $numbQuest = mysql_num_rows($quest);
        if ($numbQuest != 0) {
            ?>
            <div class="comentForm komentarisi">
                <div class="commentList">
                    <?php
                    while ($red = mysql_fetch_object($quest)) {
        ...
        <?php
        $ispodProd = mysql_query("SELECT cp.*, b.title as b_title, c.url as master_cat_url, c1.url as sub_cat_url FROM _content_proizvodi cp "
                ...
                . " WHERE cp.brand != $proizvod->brand AND (cp.status = 1 OR cp.master_status = 'Active') AND cc.category_resource_id = $catResourceID AND cp.price <= $proizvod->price ORDER BY cp.price DESC LIMIT 2") or die(mysql_error());
        $countJeftinijih = mysql_num_rows($ispodProd);
        $iznadProd = mysql_query("SELECT cp.*, b.title as b_title, c.url as master_cat_url, c1.url as sub_cat_url FROM _content_proizvodi cp "
                . " LEFT JOIN _content_brend b ON b.resource_id = cp.brand "
                . " LEFT JOIN categories_content cc ON cp.resource_id = cc.content_resource_id "
                . " LEFT JOIN categories c1 ON c1.resource_id = cc.category_resource_id "
                . " LEFT JOIN categories c ON c.resource_id = c1.parent_id "
                . " WHERE cp.brand != $proizvod->brand AND (cp.status = 1 OR cp.master_status = 'Active') AND cc.category_resource_id = $catResourceID AND cp.price >= $proizvod->price ORDER BY price ASC LIMIT 2") or die(mysql_error());
        $countSkupljih = mysql_num_rows($iznadProd);
        ...
        if ($countSkupljih < 2) {
            ...
            $ispodProd = mysql_query("SELECT cp.*, b.title as b_title, c.url as master_cat_url, c1.url as sub_cat_url FROM _content_proizvodi cp "
                    ...
                    . " LEFT JOIN categories c1 ON c1.resource_id = cc.category_resource_id "
                    . " LEFT JOIN categories c ON c.resource_id = c1.parent_id "
                    . " WHERE cp.brand != $proizvod->brand AND (cp.status = 1 OR cp.master_status = 'Active') AND cc.category_resource_id = $catResourceID AND cp.price <= $proizvod->price ORDER BY price DESC LIMIT $limit") or die(mysql_error());
        }
        if ($countJeftinijih < 2) {
            ...
            $iznadProd = mysql_query("SELECT cp.*, b.title as b_title, c.url as master_cat_url, c1.url as sub_cat_url FROM _content_proizvodi cp "
                    ...
                    . " LEFT JOIN categories c1 ON c1.resource_id = cc.category_resource_id "
                    . " LEFT JOIN categories c ON c.resource_id = c1.parent_id "
                    . " WHERE cp.brand != $proizvod->brand AND (cp.status = 1 OR cp.master_status = 'Active') AND cc.category_resource_id = $catResourceID AND cp.price >= $proizvod->price ORDER BY price ASC LIMIT $limit") or die(mysql_error());
        }
            ...
            <div id="similar" class="productHolder clear" >
                ...
                <div class="catProductAll">
                    <div  class="productListIndex row">
                        <?php
                        while ($item = mysql_fetch_object($ispodProd)) {
                            include ("little_product-search.php");
                        }
                        while ($item = mysql_fetch_object($iznadProd)) {
                            include ("little_product-search.php");
                        }
                    }
                    ?>
