...
switch ($action) {
    ...
    case "replyInbox":
        ...
        $f->sendEmail("obavestenja@sveznalica.com", "Sveznalica", $sendToEmail, "Nova poruka u sandučetu na Sveznalici!", $mailBody);
    ...
    case "sendMessage":
        ...
        $i_friend = $f->getValue("i_f");
        $subject = $f->getValue("s");
        $body = $f->getValue("b");
        ...
        $subject = ($subject == "") ? "(nema naslova)" : $subject;

        $db->execQuery("INSERT INTO messages (`author`, `receiver`, `subject`, `body`, `date`) VALUES ('$userId', '$i_friend', '$subject', '$body', '$dateNow')");
        echo 1;

        $sendToEmail = $db->getValue("email", "users", "id", $i_friend);
        $sendFrom = $db->getValue("username", "users", "id", $userId);

        $mailBody = "Imate novu poruku u sandučetu od <strong>$sendFrom</strong>!<br><br>" .
                "\"<quote>" . nl2br($body) . "</quote>\"<br><br>" .
                "Posetite http://www.sveznalica.com/inbox/ i pročitajte!<br><br>" .
                "Vaša Sveznalica";
    ...
    case "lostPass":
        ...
        if ($user->id != "") {
            ...
            $f->sendEmail($settings->site_email, $settings->site_domain, $email, "Promena šifre", $text);
