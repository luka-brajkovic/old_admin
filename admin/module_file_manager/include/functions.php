...
function get_languages() {
    ...
    foreach ($langfiles as $lang) {
        if (stristr($lang, '_mimes') || $lang == 'index.html')
            continue;
...
function get_mime_type($abs_item, $query) { // get file's mimetype
    ...
    if ((function_exists("is_executable") &&
            ...
            @eregi($GLOBALS["super_mimes"]["exe"][2], $abs_item)) {      // executable
...
function get_show_item($dir, $item) {  // show this file?
    ...
    if ($item == "." || $item == ".." ||
            (substr($item, 0, 1) == "." && $GLOBALS["show_hidden"] == false))
        return false;
...
function get_dir_list($dir = '') {
    ...
    foreach ($files as $item) {
        ...
        if (get_is_dir($item)) {
            $index = str_replace(
                    str_replace('\\', '/', $GLOBALS['home_dir'] . $GLOBALS['separator']), '', $itemname);
...
function ext_alertBox($msg) {
    return ext_scriptTag('', 'Ext.Msg.alert( \'' . $GLOBALS["error_msg"]['message'] . '\', \'' . @mysqli_escape_string($msg) . '\' );');
}

function ext_successBox($msg) {
    return ext_scriptTag('', 'Ext.msgBoxSlider.msg( \'' . ext_Lang::msg('success', true) . '\', \'' . @mysqli_escape_string($msg) . '\' );');
}
