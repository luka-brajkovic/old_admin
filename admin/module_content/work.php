...
switch ($action) {
    ...
    case "edit_content":
        ...
        if ($contentType->category_type != 0) {
            ...
            if ($category != "") {
                ...
                if (is_file("../../admin/module_filters/save_joins.php")) {


                    include("../../admin/module_filters/save_joins.php");
                }
            } else {
        ...
        foreach ($languages as $key => $language) {
            ...
            foreach ($contentCollection as $item) {
                ...
                if ($content_type_id == 81) {
                    ...
                    $emailName = $languageValues["title"];
                    $proizvodRid = $languageValues["proizvod"];

                    $proizvod = mysqli_query($conn, "SELECT "
                            . " cp.url, cp.title, cp.resource_id, c.url as cat_master_url, c1.url as cat_url, cb.title as b_title FROM _content_products cp "
                            ...
                            . " WHERE cp.resource_id = $proizvodRid LIMIT 1") or die(mysqli_error($conn));
                    $proizvod = mysqli_fetch_object($proizvod);

                    $bodySend = 'Odgovor na Vaše pitanje na proizvod "' . $proizvod->b_title . ' ' . $proizvod->title . '" možete pročitati na linku ispod:<br/><br/> <a href="' . $csDomain . $proizvod->cat_master_url . '/' . $proizvod->cat_url . '/' . $proizvod->url . '/' . $proizvod->resource_id . '" title="' . $proizvod->b_title . ' ' . $proizvod->title . '">' . $csDomain . $proizvod->cat_master_url . '/' . $proizvod->cat_url . '/' . $proizvod->url . '/' . $proizvod->resource_id . '</a>';
                }
                    ...
                    switch ($field->field_type) {
                        ...
                        case "radio":
                            if ($content_type_id == 81 && $languageValues["odgovor"] == '' || $item->vreme_odgovora == "0000-00-00 00:00:00") {
                                ...
                                $f->sendMail($csEmail, $csName, $emailSend, $emailName, $csName . " | Odgovor na pitanje", $bodySend, $currentLanguage);
                            }
                        ...
                        case "image":
                            ...
                            if (isset($_FILES[$fieldName]) && $_FILES[$fieldName]['size'] > 0) {
                                ...
                                $image_name = $f->fileUpload($fieldName, "../../uploads/uploaded_pictures/$filesTableName/", "$item->url-$rand", PIC_EXT);
                        ...
                        case "file":
                            ...
                            if (isset($_FILES[$fieldName]) && $_FILES[$fieldName]['size'] > 0) {
                                ...
                                $file_name = $f->fileUpload($fieldName, "../../uploads/uploaded_files/$filesTableName/", "$item->url-$rand", FILE_EXT);
    ...
    case "delete_content":
        ...
        mysqli_query($conn, "DELETE FROM $resource->table_name WHERE resource_id = $resource_id");
        mysqli_query($conn, "DELETE FROM resources WHERE id = $resource_id");
    ...
    case "copy_content":
        ...
        foreach ($languages as $language) {
            ...
            $maxValuOrder = mysqli_query($conn, "SELECT MAX(ordering) as max_order FROM $ct->table_name");
            $maxValuOrder = mysqli_fetch_object($maxValuOrder);
    ...
    case "povecaj_cenu":
        ...
        foreach ($resIDs as $rid) {
            mysqli_query($conn, "UPDATE _content_products SET `price` = round((($procenat * price) / 100) + price), `old_price` = round((($procenat * old_price) / 100) + old_price) WHERE resource_id = $rid");
        }
    ...
    case "editable":
        ...
        mysqli_query($conn, "UPDATE $tableName SET $columnName = '$value' WHERE resource_id = $rid");
    ...
    case "save_title":
        ...
        $ct = new View('content_types', $cid);
        $tableName = $ct->table_name;
        mysqli_query($conn, "UPDATE $tableName SET title = '$value', url = '$url' WHERE resource_id = $rid") or die(mysqli_error($conn));
    ...
    case "save_status":
        ...
        $ct = new View('content_types', $cid);
        $tableName = $ct->table_name;
        mysqli_query($conn, "UPDATE $tableName SET status = '$value' WHERE resource_id = $rid") or die(mysqli_error($conn));
