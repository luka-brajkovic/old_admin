...
class ext_Chmod extends ext_Action {
    ...
    function execAction($dir, $item) {  // change permissions
        ...
        "dialogtitle": "<?= ext_Lang::msg('actperms') ?>",
        "title" : "<?= $text ?>",
        "frame": true,
        "items": [{
        "layout": "column",
        ...
        for ($i = 0; $i < 3; ++$i) {
            ?>
            "width":80, 
            "title":"<?= ext_Lang::msg(array('miscchmod' => $i), true) ?>",					
            "items": [{
            <?php
            for ($j = 0; $j < 3; ++$j) {
                ?>
        ...
        "xtype": "checkbox",
        "fieldLabel":"<?= ext_Lang::msg('recurse_subdirs', true) ?>",
        "name":"do_recurse"
        }],
        "buttons": [{
        "text": "<?= ext_Lang::msg('btnsave', true) ?>", 
        "handler": function() {
        statusBarMessage( '<?= ext_Lang::msg('permissions_processing', true) ?>', true );
        ...
        "action": "chmod", 
        "dir": "<?= stripslashes($GLOBALS['__POST']["dir"]) ?>", 
        "selitems[]": ['<?= implode("','", $GLOBALS['__POST']["selitems"]) ?>'], 
        confirm: 'true'
        }
        });
        }
