...
class Functions extends Database {
    ...
    function printLanguages() {
        ...
        ?>
        <form method="POST" action="/admin/work.php">
    ...
    function listInbox($SQL, $limit, $page) {
        ...
        while ($data = mysqli_fetch_array($upit, MYSQLI_ASSOC)) {
        ...
        <ul>
            ...
            while ($data = mysqli_fetch_array($query)) {
                ...
                <li <?php if ($selected == $data['id']) echo "class='active'"; ?>>
                    <a href="/admin/module_content/index.php?sort_category=<?= $data['resource_id']; ?>&cid=<?= $type; ?>" class="tooltip" title="Category id=<?= $data['id']; ?>"><?= $data['title']; ?> (<?= $num; ?>)</a>
    ...
    function printContentsForSidebar($content_type) {
        ...
        <li <?php if ($module_name == "content" && $cid == $ct->id) echo "class='active'"; ?>>
            <a href="/admin/module_content/index.php?cid=<?= $content_type->id; ?>" class="tooltip" title="Manage <?= $content_type->title; ?>"><?= $content_type->title; ?>&nbsp;(<?= ($contentsCollection->resultCount > 0) ? $contentsCollection->resultCount : "empty"; ?>)</a>
        ...
        /* function printCategoryContentRec($category){
          ...
          ?>
          <a href="/admin/module_content/index.php?cid=<?= $content_type->id; ?>&category_id=<?= $category->id; ?>" class="tooltip" title="Manage <?= $content_type->title; ?>"><?= $category->title; ?> <?php if($cat_contCollection->resultCount > 0)  echo "(" . $cat_contCollection->resultCount . ")"; ?></a>
          <?php
          } else {
          ?>
          <a href="/admin/module_categories/index.php?cid=<?= $content_type->id; ?>&parent_id=<?= $category->resource_id; ?>" class="tooltip" title="Manage <?= $content_type->title; ?>"><?= $category->title; ?> <?php if($cat_contCollection->resultCount > 0)  echo "(" . $cat_contCollection->resultCount . ")"; ?></a>
          <?php
          }
        ...
        function printCategoryLinks($category) {
            ...
            if ($cat_contCollection->resultCount == 0) {
                ?>
            <li <?php if ($module_name == "content" && $cid == $ct->id) echo "class='active'"; ?>>	
                <a href="/admin/module_categories/index.php?parent_id=<?= $category->resource_id ?>&cid=<?= $category->content_type_id ?>"><?= $category->title ?></a>	  		
                <?php
            } else {
                ?>
            <li <?php if ($module_name == "content" && $cid == $ct->id) echo "class='active'"; ?>>	
                <a href="/admin/module_content/index.php?category_id=<?= $category->id ?>&cid=<?= $category->content_type_id ?>"><?= $category->title ?></a>
                <?php
            }
        ...
        function printCategoryRec($category) {
            ...
            if ($categoriesCollection->resultCount == 0) {
                ...
                if ($cat_contCollection->resultCount > 0) {
                    ?>	
                    <a href="/admin/module_content/index.php?category_id=<?= $category->id ?>&cid=<?= $category->content_type_id ?>"><?= $category->title . " (" . $cat_contCollection->resultCount . " content)"; ?></a>
                    <?php
                } else {
                    ?>
                    <a href="/admin/module_categories/index.php?parent_id=<?= $category->resource_id ?>&cid=<?= $category->content_type_id ?>"><?= $category->title . " (empty)" ?></a>
                    <?php
                }
            } else if ($categoriesCollection->resultCount > 0) {
                ?>
                <a href="/admin/module_categories/index.php?parent_id=<?= $category->resource_id ?>&cid=<?= $category->content_type_id ?>"><?= $category->title . " (" . $categoriesCollection->resultCount . " subcat)" ?></a>
                <?php
                foreach ($categories as $categorySingle)
