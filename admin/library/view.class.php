        ...
        if ($this->key_value !== false) {
            ...
            foreach ($fields as $key => $field_info) {
                if ($field_info['Field'] != $this->key_field) {
                    $SQL .= "`" . $field_info['Field'] . "` = '" .  mysqli_real_escape_string($this->dbLink, $this->{$field_info['Field']}) . "', ";
                }
            }
            ...
            foreach ($fields as $key => $field_info) {
                if ($field_info['Field'] != $this->key_field) {
                    $SQL .= "`" . mysqli_real_escape_string($this->dbLink, $field_info['Field']) . "`, ";
                }
            }
            $SQL = substr($SQL, 0, strlen($SQL) - 2);
            $SQL .= ") VALUES (";
            foreach ($fields as $key => $field_info) {
                if ($field_info['Field'] != $this->key_field) {
                    $SQL .= "'" . mysqli_real_escape_string($this->dbLink, $this->{$field_info['Field']}) . "', ";
                }
            }
            $SQL = substr($SQL, 0, strlen($SQL) - 2);
            $SQL .= ")";
