...
foreach ($inProd as $product) {
    ...
    foreach ($dimenzije_array as $dim) {
        foreach ($slike_array as $slika) {
            if ($slika != "") {
                if (is_file("/home/skycompu/public_html/uploads/uploaded_pictures/_content_proizvodi/$dim/$slika")) {
                    $obrisano++;
                    unlink("/home/skycompu/public_html/uploads/uploaded_pictures/_content_proizvodi/$dim/$slika");
                }
            }
        }
    }
    if (is_file("/home/skycompu/public_html/uploads/uploaded_pictures/_content_proizvodi/$dimUrlLit/$product->product_image")) {
        $obrisano++;
        unlink("/home/skycompu/public_html/uploads/uploaded_pictures/_content_proizvodi/$dimUrlLit/$product->product_image");
    }
}
