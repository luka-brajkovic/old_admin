...
class ext_Archive extends ext_Action {
    ...
    function execAction($dir) {
        ...
        "dialogtitle": "<?= $GLOBALS["messages"]["actarchive"] ?>",
        "frame": true,
        "items": [{
        "xtype": "textfield",
        "fieldLabel": "<?= ext_Lang::msg('archive_name', true) ?>",
        "name": "name",
        "value": "<?= $GLOBALS['item'] . '.' . $default_archive_type ?>",
        "width": "200"
        },
        {
        "xtype": "combo",
        "fieldLabel": "<?= ext_Lang::msg('typeheader', true) ?>",
        "store": [
        ['zip', 'Zip (<?= ext_Lang::msg('normal_compression', true) ?>)'],
        ['tgz', 'Tar/Gz (<?= ext_Lang::msg('good_compression', true) ?>)'],
        <?php
        if (extension_loaded("bz2")) {
            echo "['tbz', 'Tar/Bzip2 (" . ext_Lang::msg('best_compression', true) . ")'],";
        }
        ?>
        ['tar', 'Tar (<?= ext_Lang::msg('no_compression', true) ?>)']
        ],
        "displayField":"typename",
        "valueField": "type",
        "name": "type",
        "value": "<?= $default_archive_type ?>",
        "triggerAction": "all",
        "hiddenName": "type",
        ...
        "xtype": "textfield",
        "fieldLabel": "<?= ext_Lang::msg('archive_saveToDir', true) ?>",
        "name": "saveToDir",
        "value": "<?= str_replace("'", "\'", $dir) ?>",
        "width": "200"
        },{
        "xtype": "checkbox",
        "fieldLabel": "<?= ext_Lang::msg('downlink', true) ?>?",
        "name": "download",
        "checked": "true"
        }
        ],
        "buttons": [{
        "text": "<?= ext_Lang::msg('btncreate', true) ?>", 
        "type": "submit", 
        "handler": function() { 
        Ext.ux.OnDemandLoad.load( "<?= $GLOBALS['script_name'] ?>?option=com_extplorer&action=include_javascript&file=archive.js", 
