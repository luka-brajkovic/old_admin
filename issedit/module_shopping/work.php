...
switch ($action) {
    ...
    case "change_admin_zaposleni":
        ...
        $result = mysql_query("SHOW COLUMNS FROM `korpa` LIKE 'admin_zaposleni'");
        $exists = (mysql_num_rows($result)) ? TRUE : FALSE;
        if (!$exists) {
            mysql_query("ALTER TABLE `korpa` ADD admin_zaposleni VARCHAR( 255 ) ") or die(mysql_error());
            mysql_query("ALTER TABLE `korpa` CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci") or die(mysql_error());
        }
    ...
    case "change_kolicina":
        ...
        $ostaloUKorpi = mysql_query("SELECT * FROM proizvodi_korpe WHERE korpa_rid = $korpaID");
