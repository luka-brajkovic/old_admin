...
class Collection extends Functions {
    ...
    function getCollection($whereSQL = "", $orderSQL = "") {

        $limitSQL = "";
        $limitSQL = ($this->_limit != "") ? " LIMIT " . $this->_limit . (($this->_offset != "") ? ", " . $this->_offset : "") : "";

        $this->totalCount = Database::numRows("SELECT * FROM `" . $this->table . "`");
        $query = Database::execQuery("SELECT * FROM `" . $this->table . "` $whereSQL $orderSQL" . $limitSQL);
        $this->resultCount = mysqli_num_rows($query);
