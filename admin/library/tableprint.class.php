...
class Tableprint extends Functions {
                        ...
                        <tr id="item_<?= $category->resource_id; ?>" <?php if ($category->status == 2) echo 'style="background:#ffe5eb"'; ?>>
                            ...
                            <td>
                                ...
                                <?php if ($contentCount > 0) { ?>
                                    <a title="View and order content for category: <?= $category->title; ?>" class="button_table tooltip handle" href="/admin/module_content/index.php?cid=<?= $category->content_type_id; ?>&category_id=<?= $category->resource_id; ?>">
    ...
    function printContentTable($cid, $wholeQuery, $tableCustomFields, $start_from, $img) {
        ...
        /*
          echo $wholeQuery . "<br>";
         */
        $items = mysqli_query($this->dbLink,$wholeQuery);
        $rbr = $start_from;
                ...
                <tbody>
                    ...
                    while ($item = mysqli_fetch_object($items)) {
                        ...
                        <tr id="item_<?= $item->resource_id; ?>" <?php if ($item->status == 2) echo 'style="background:#ffe5eb"'; ?>>
                            ...
                            <?php
                            if ($cid == 72) {
                                $brandSingle = mysqli_query($this->dbLink,"SELECT title FROM _content_brand WHERE resource_id = $item->brand LIMIT 1");
                                $brandSingle = mysqli_fetch_object($brandSingle);
                                ?>
            ...
            <tbody>
                ...
                foreach ($carts as $cart) {
                    ...
                    <tr <?php if ($cart->status == 0) echo 'style="background:#ffe5eb"'; ?>>
                        <td><?= $cart->id; ?>	</td>
                        <?php
                        $userKorpa = new View("_content_users", $cart->user_id, "id");
                        ?>
                        <td><?php
                        ...
                        <?php
                        $city = new View("_content_cities", $cart->grad, "resource_id");
                        ?>
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="fullwidth rezultati">
            ...
            <tbody>
                ...
                while ($dataKorpa = mysqli_fetch_array($proizvodiKorpe)) {
                    ...
                    <tr>
                        <td>
                            <?php if (is_file("../../uploads/uploaded_pictures/_content_products/" . $dimUrlLit . "/" . $dataKorpa['slika'])) { ?>
                                <img style="widht:auto; height: 50px;" src="/uploads/uploaded_pictures/_content_products/<?= $dimUrlLit . "/" . $dataKorpa['slika']; ?>" alt="<?= $dataKorpa['title']; ?>" title="<?= $dataKorpa['title']; ?>" />
                            <?php } else if (is_file("../../uploads/uploaded_pictures/_content_products/" . $dimUrlLitSecund . "/" . $dataKorpa['slika'])) { ?>
                                <img style="widht:auto; height: 50px;" src="/uploads/uploaded_pictures/_content_products/<?= $dimUrlLitSecund . "/" . $dataKorpa['slika']; ?>" alt="<?= $dataKorpa['title']; ?>" title="<?= $dataKorpa['title']; ?>" />
                            <?php } else if (is_file("../../uploads/uploaded_pictures/_content_products/" . $dimUrlLitThird . "/" . $dataKorpa['slika'])) { ?>
                                <img style="widht:auto; height: 50px;" src="/uploads/uploaded_pictures/_content_products/<?= $dimUrlLitThird . "/" . $dataKorpa['slika']; ?>" alt="<?= $dataKorpa['title']; ?>" title="<?= $dataKorpa['title']; ?>" />
                            <?php } ?> 
                    ...
                    <?php
                    if ($dataKorpa['gratis']) {
                        $gratis = mysqli_query($this->dbLink,"SELECT cp.product_image, cp.resource_id, cp.title, cp.url, c.url as sub_url, c1.url as cat_url, cb.title as b_title FROM _content_products cp "
                        ...
                        <tr>
                            <td>
                                <a href="/<?= $gratis->sub_url . "/" . $gratis->cat_url . "/" . $gratis->url . "/" . $gratis->resource_id; ?>" title="<?= $gratis->b_title . " " . $gratis->title; ?>">
                                    <img style="widht:auto; height: 50px;" src="/uploads/uploaded_pictures/_content_products/<?php echo $dimUrlLitSecund . "/" . $gratis->product_image; ?>" alt="<?= $gratis->b_title . " " . $gratis->title; ?>">
                                </a>
                            </td>
