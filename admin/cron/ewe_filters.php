...
$external_xml = "https://www.ewe.rs/share/backend_231/?user=skycomp&secretcode=322e8&attributes=1";
...
<html lang="sr">
    ...
    foreach ($xml->children() as $childs) {
        ...
        $MasterCat = mysqli_query($conn,"SELECT resource_id, title FROM categories WHERE title = '$category' AND parent_id = '0' LIMIT 1");
        $MasterCat = mysqli_fetch_object($MasterCat);

        $subCat = mysqli_query($conn,"SELECT resource_id, title FROM categories WHERE title = '$subcategory' AND parent_id = '$MasterCat->resource_id' LIMIT 1");
        $subCat = mysqli_fetch_object($subCat);

        $id = htmlspecialchars($id);
        $inItem = new View("_content_products", "$id", "product_code");
        $inFilter = mysqli_query($conn,"SELECT id FROM filter_joins WHERE product_rid = '$inItem->resource_id' LIMIT 1");
        $inFilter = mysqli_fetch_object($inFilter);
        ...
        if (!empty($inItem->id) && empty($inFilter->id)) {
            if (count($osnovne_karakteristike) > 0) {
                ...
                foreach ($osnovne_karakteristike as $key => $value) {
                    ...
                    $kecHeader = mysqli_query($conn,"SELECT id FROM filter_headers WHERE url = '$headerUrl' AND cat_resource_id = '$subCat->resource_id' LIMIT 1");
                    $kecHeader = mysqli_fetch_object($kecHeader);
                    ...
                    $kecValue = mysqli_query($conn,"SELECT id FROM filter_values WHERE title = '$value' AND fh_id = '$kecHeader->id' LIMIT 1");
                    $kecValue = mysqli_fetch_object($kecValue);
    ...
    $brisiFiltere = 0;
    $zaobrisati = mysqli_query($conn,"SELECT cp.resource_id as cprid, fj.id as fjid FROM _content_products cp LEFT JOIN filter_joins fj ON fj.product_rid = cp.resource_id WHERE cp.status = 2 AND cp.master_status != 'Active'");
    while ($zaobrisatProduct = mysqli_fetch_object($zaobrisati)) {
        if ($zaobrisatProduct->fjid != "") {
            $brisiFiltere++;
            mysqli_query($conn,"DELETE FROM filter_joins WHERE id = '$zaobrisatProduct->fjid'");
        }
    }
