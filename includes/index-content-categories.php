...
<ul class="list categoriesList quarterMarginMasterR">
    <?php
    $proizvodi_na_akciji = mysqli_query($conn,"SELECT cp.*,c1.title as cat_title,c1.url as cat_url FROM _content_products cp"
            ...
            . " WHERE (cp.status = 1 OR cp.master_status = 'Active') AND c1.parent_id = 0 AND (cp.akcija = 'Da' OR (cp.price < cp.old_price OR cp.master_price < cp.old_price)) GROUP BY c1.resource_id ORDER BY c1.title ") or die(mysqli_error($conn));
    if (mysqli_num_rows($proizvodi_na_akciji) > 0) {
        ?>
        <li>
    ...
    <?php } ?>
    <?php
    $items = mysqli_query($conn,"SELECT * FROM categories WHERE status = 1 AND parent_id = '0' AND content_type_id = 72 ORDER BY ordering");
    $counter3 = 0;
    $counter4 = 0;
    while ($item = mysqli_fetch_object($items)) {
        $subItems = mysqli_query($conn,"SELECT * FROM categories WHERE status = 1 AND parent_id = '$item->resource_id' AND content_type_id = '72' ORDER BY ordering");
        ?>
        <li>
            <?php if (mysqli_num_rows($subItems) > 0) { ?>
                <a title="<?= $item->title; ?>" href="/<?= $item->url; ?>" class="<?= ($catMasterDataURL == $item->url) ? "active" : ""; ?> have_sub"><?= $item->title; ?></a>
                <ul class="sub">
                    <?php
                    while ($subItem = mysqli_fetch_object($subItems)) {
                        $checkLast = mysqli_query($conn,"SELECT c3.resource_id as last_cat_rid FROM categories c3 "
                                ...
                                . " WHERE (cp.status = 1 OR cp.master_status = 'Active') AND c2.resource_id = '$subItem->resource_id' GROUP BY (c3.resource_id) LIMIT 1") or die(mysqli_error($conn));
                        if (mysqli_num_rows($checkLast) > 0) {
                            ...
                            $subCatAlls = mysqli_query($conn,"SELECT c2.resource_id as cat_rid FROM categories c2 "
                                    ...
                                    . " LEFT JOIN _content_products cp ON cp.resource_id = cc.content_resource_id "
                                    . " WHERE (cp.status = 1 OR cp.master_status = 'Active') AND c2.resource_id = '$subItem->resource_id' GROUP BY (c2.resource_id) LIMIT 1") or die(mysqli_error($conn));
