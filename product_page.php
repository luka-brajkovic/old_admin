...
<meta property="product:category" content="<?= $niz[2]->title; ?>" /><?php if ($proizvod->master_status != 'Active' && $proizvod->status != 1) { ?>
	<meta property="product:availability" content="oos" /><?php } else {
	?>
...
<body>
	...
	<?php if ($uso != '') { ?>
		<div id="popup">
			<div id='popupInner'>
				<?php
				if (!empty($greske)) {
					$_SESSION['infoTitle'] = "<h1>Proverite podatke</h1>";
					$_SESSION['infoText'] = "<p>Niste postavili komentra, molimo Vas proverite unete podatke!</p>";
				} elseif (!empty($komentarisano)) {
					$_SESSION['infoTitle'] = "<h1>Uspešno</h1>";
					$_SESSION['infoText'] = "<p>Uspešno ste postlali pitanje ili komentar. Uskoro će ga odobriti administrator i bićete obavešteni putem email-a.</p>";
				}
				?>
	...
	include_once ("includes/product_page-content.php");
	include_once ("footer.php");
	?>
    <script type="text/javascript">function openTab(a, b) {
			var c, d, e;
