                            ...
                            if ($header->show == 1) {
                                ...
                                if (count($filter_values_arr) > 0) {
                                    foreach ($filter_values_arr as $value) {
                                        $queryIf = mysql_query("SELECT * FROM filter_joins WHERE cat_rid = $selectedCategory AND lang = $lang AND product_rid = $resource_id AND fh_id = $header->id AND fv_id = $value->id") or die(mysql_error());
                                        $dataJoin = mysql_fetch_object($queryIf);
