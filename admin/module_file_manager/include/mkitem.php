...
class ext_Mkitem extends ext_Action {
    ...
    function execAction($dir) {  // make new directory or file
        ...
        "xtype": "combo",
        "fieldLabel": "Type",
        "store": [["file", "<?= ext_Lang::mime('file', true) ?>"],
        ["dir", "<?= ext_Lang::mime('dir', true) ?>"]
        <?php if (!ext_isFTPMode() && !$GLOBALS['isWindows']) { ?>
            ,["symlink", "<?= ext_Lang::mime('symlink', true) ?>"]
        <?php }
        ?>
        ],
        displayField:"type",
        ...
        "xtype": "textfield",
        "fieldLabel": "<?= ext_Lang::msg('symlink_target', true) ?>",
        "name": "symlink_target",
        "width":175,
        "allowBlank":true
        }],
