...
$langsQu = mysqli_query($conn, "SELECT * FROM languages WHERE code!='' ");
...
if (mysqli_num_rows($langsQu) > 1) {
    ?>
    <select name='lang' id='lang'>
        <?php while ($thisLang = mysqli_fetch_object($langsQu)) {
            ?>
...
foreach ($fieldsArr as $field) {
    ...
    $this_sort = "";
    if (isset($_GET['sort_' . $field->column_name]) && $_GET['sort_' . $field->column_name] != '') {
...
<br/><br/>
<?php
$numProds = mysqli_num_rows(mysqli_query($conn, $bigQuery));
