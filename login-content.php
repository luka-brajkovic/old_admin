...
if ($f->verifyFormToken('form1')) {
    ...
    if (empty($greske)) {
        $userData = new View("_content_korisnici", $email, "e-mail");
        if ($userData->id) {
            if ($userData->lozinka != md5($pass)) { /* OVDE PROMENI KAKO SE ZOVE LOZINKA U ADMINU */
                ...
                $my_query = mysql_query("SELECT `e-mail` FROM _content_korisnici WHERE id = $userData->id");
                $email_neaktivan = mysql_fetch_array($my_query);
...
<div class="container">
    ...
    <h1>Dobrodošli na <?= $configSiteFirm; ?><a class="right" style="line-height: 34px;" title="<?= $configSiteDomain; ?>" href="/">Natrag na naslovnu stranicu</a></h1>
    <div class="row">
        ...
        <div class="third-x2">
            <div class="inner pictureRight">
                <?= ($db->getValue("text", "_content_html_blocks", "url", "desno-prijavljivanje") != '') ? $db->getValue("text", "_content_html_blocks", "url", "desno-prijavljivanje") : ""; ?>
            </div>    
        </div>
    </div>
</div>