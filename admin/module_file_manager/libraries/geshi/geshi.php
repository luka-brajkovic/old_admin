...
class GeSHi {
    ...
    function error() {
        if ($this->error) {
            ...
            $msg = str_replace(
                    array_keys($debug_tpl_vars), array_values($debug_tpl_vars), $this->error_messages[$this->error]);
        ...
        if ($this->lexic_permissions['NUMBERS']) {
            ...
            if (is_array($this->language_data['NUMBERS'])) {
                ...
                if (!$this->language_data['NUMBERS']) {
                    $this->language_data['NUMBERS'] = GESHI_NUMBER_INT_BASIC |
                                ...
                                if ($es_char == "\n") {
                                    ...
                                    if (function_exists('mb_substr')) {
                                        ...
                                        if (preg_match("/[\xC2-\xDF][\x80-\xBF]" .
                                                        ...
                                                        "|[\xF1-\xF3][\x80-\xBF]{3}" .
                                                        "|\xF4[\x80-\x8F][\x80-\xBF]{2}/s", $part, $es_char_m, null, $es_pos + 1)) {
                ...
                if (is_array($regexp)) {
                    if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
                        ...
                        $stuff_to_parse = preg_replace_callback(
                                "/" . $regexp[GESHI_SEARCH] . "/{$regexp[GESHI_MODIFIERS]}", array($this, 'handle_multiline_regexps'), $stuff_to_parse);
                        $this->_hmr_replace = false;
                        $this->_hmr_before = '';
                        $this->_hmr_after = '';
                    } else {
                        $stuff_to_parse = preg_replace(
                                '/' . $regexp[GESHI_SEARCH] . '/' . $regexp[GESHI_MODIFIERS], $regexp[GESHI_BEFORE] . '<|!REG3XP' . $key . '!>' . $regexp[GESHI_REPLACE] . '|>' . $regexp[GESHI_AFTER], $stuff_to_parse);
                    }
                } else {
                    if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
                        // produce valid HTML when we match multiple lines
        ...
        foreach (array_keys($this->language_data['KEYWORDS']) as $k) {
            if (!isset($this->lexic_permissions['KEYWORDS'][$k]) ||
                ...
                for ($set = 0, $set_length = count($this->language_data['CACHED_KEYWORD_LISTS'][$k]); $set < $set_length; ++$set) {
                    ...
                    $stuff_to_parse = preg_replace_callback(
                            "/$disallowed_before_local({$keywordset})(?!\<DOT\>(?:htm|php))$disallowed_after_local/$modifiers", array($this, 'handle_keyword_replace'), $stuff_to_parse
                    );
                }
            }
        }
