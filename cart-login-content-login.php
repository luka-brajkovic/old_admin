...
if ($isLoged) {
    ...
    if ($f->getValue("hitted") != '') {
        while ($field = mysql_fetch_object($user_fields)) {
            ...
            switch ($columnName) {
                ...
                case "e-mail":
                    ...
                    $emailCheck = mysql_query("SELECT resource_id FROM _content_korisnici WHERE `e-mail` = '$email' LIMIT 1");
                    $emailCheck = mysql_fetch_object($emailCheck);
        ...
        if (empty($greske)) {
            ...
            $body .= "<p>Vaš <a href='".$configSiteDomain."' title='$configSiteFirm'>" . $configSiteFirm . "</a> tim</p>";
            
            $f->sendEmail($configSiteEmail, "$configSiteFirm | Internet prodavnica", $email, "Dobrodošli", $body);

            $user = new View("_content_korisnici", $resource->id, 'resource_id');
        ...
        <div class="clear logCont">
            <div class="row">
                ...
                <div class="half" style="padding-right: 20px; box-sizing: border-box;text-align: center;">
                    <h3 style="margin-bottom: 30px;">Ukoliko imate nalog na "<?= $configSiteFirm; ?>" klikninite na dugme ispod i prijavite se zbog lakše kupovine.</h3>
                ...
                <div class="half">
                    <div class="gray">
                        <?php
                        $user_fields = mysql_query("SELECT * FROM content_type_fields WHERE content_type_id = $contentTypeIDUsers AND column_name != 'lozinka' AND column_name != 'fiksni_telefon' ORDER BY ordering");
                        /*
