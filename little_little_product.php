    ...
    <div class="littleLittleProductAll clear">
        <figure class="third left">
            <a href="/<?= $item->master_cat_url . "/" . $item->sub_cat_url . "/" . $item->url . "/" . $item->resource_id; ?>" title='<?= htmlspecialchars_decode($item->title); ?>'>
                <?php if (is_file("uploads/uploaded_pictures/_content_products/" . $dimUrlLit . "/" . $item->product_image)) { ?>
                    <img class="transition" src="/uploads/uploaded_pictures/_content_products/<?= $dimUrlLit . "/" . $item->product_image; ?>" alt='<?= htmlspecialchars_decode($item->title); ?> slika' title='<?= htmlspecialchars_decode($item->title); ?>' />
                <?php } elseif (is_file("uploads/uploaded_pictures/_content_products/" . $dimUrlLitSecund . "/" . $item->product_image)) { ?>
                    <img class="transition" src="/uploads/uploaded_pictures/_content_products/<?= $dimUrlLitSecund . "/" . $item->product_image; ?>" alt='<?= htmlspecialchars_decode($item->title); ?> slika' title='<?= htmlspecialchars_decode($item->title); ?>' />
                <?php } else { ?>
        ...
        <div class="third-x2 right">
            <div class="littleLittleDesc quarterMarginLB">
                ...
                <?php if ($item->old_price != '' && $item->old_price > 0) { ?> <span><?= number_format($item->old_price, 0, ",", "."); ?></span><?php } ?>
                <strong<?php
                if (($item->old_price != '' && $item->old_price > 0) || $item->akcija == "Da") {
                    echo " class='popustPrice'";
                }
                ?>><?= ($item->master_price > 0) ? number_format($item->master_price, 0, ",", ".") : number_format($item->price, 0, ",", "."); ?> rsd</strong>
            </div>
        </div>
    </div>
</div>