...
/**
 * @version $Id: admin.php 199 2011-06-18 03:46:10Z sloarch07 $
 * @package eXtplorer
...
function admin($admin, $dir) {
    ...
    "width": "450",
    "id": "dialog_tabpanel",
    "dialogtitle": "<?= ext_Lang::msg('actadmin') ?>",
    "listeners": {
    "afterrender": {
    fn: function(cmp) {
    cmp.activate(<?= $activeTab ?>);
    }
    }
    },
    "items":
    ...
    "title": "<?= ext_Lang::msg('actchpwd', true) ?>",
    "frame": true,
    "items": [{
    "xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('miscoldpass', true) ?>",
    "name": "oldpwd",
    "inputType": "password",
    "allowBlank":false
    },
    {	"xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('miscnewpass', true) ?>",
    "name": "newpwd1",
    "hiddenName": "newpwd1",
    "inputType": "password",
    "allowBlank":false
    ...
    { 	"xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('miscconfnewpass', true) ?>",
    "name": "newpwd2",
    "hiddenName": "newpwd2",
    "inputType": "password",
    "allowBlank":false
    ...
    if ($admin) {
        ...
        "labelWidth": 125,
        "url":"<?= basename($GLOBALS['script_name']) ?>",
        title: "<?= ext_Lang::msg('actusers', true) ?>",
        ...
        for ($i = 0; $i < $cnt; ++$i) {
            ...
            "xtype": "radio",
            "name": "nuser",
            "inputValue": "<?= $GLOBALS["users"][$i][0] ?>",
            "fieldLabel": "<?= $user ?>",
            "boxLabel": "<?=
            '<strong>Homedir:</strong> ' . $home . '; '
        ...
        "text": "<?= ext_Lang::msg('btnadd', true) ?>", 
        "handler": function() {
        Ext.Ajax.request( { url: "<?= basename($GLOBALS['script_name']) ?>",
        ...
        Ext.Msg.alert( "<?= ext_Lang::err('error', true) ?>", json.error );
        dialog.destroy();
        return false;
        }
        ...
        {
        "text": "<?= ext_Lang::msg('btnedit', true) ?>", 
        "handler": function() {
        frm =  Ext.getCmp("userlist").getForm();
        try {
        theUser = frm.findField(0).getGroupValue();
        } catch(e) {
        Ext.Msg.alert( "Error", "<?= ext_Lang::err('miscselitems', true) ?>" );
        return;
        }
        Ext.Ajax.request( { url: "<?= basename($GLOBALS['script_name']) ?>",
        ...
        Ext.Msg.confirm( "", String.format( "<?= ext_Lang::err('miscdeluser', true) ?>", theUser ), function( btn ) {
        if( btn != 'yes') return;
        statusBarMessage( "Please wait...", true );
        frm.submit({
        "success": function(form, action) {
...
function show_userform($data = null) {
    ...
    items: [{
    "xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('miscusername', true) ?>",
    "name": "nuser",
    "value": "<?= @$data[0] ?>",
    "width":175,
    "allowBlank":false
    ...
    "xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('miscconfpass', true) ?>",
    "name": "pass1",
    "inputType": "password",
    "width":175
    },
    ...
    <?php if (!empty($data[0])) { ?>
        {	"xtype": "checkbox",
        "fieldLabel": "<?= ext_Lang::msg('miscchpass', true) ?>",
        "name": "chpass",
        "hiddenValue": "true"
        },
    <?php }
    ...
    {
    "xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('mischomedir', true) ?>",
    "name": "home_dir",
    "value": "<?= !empty($data[2]) ? $data[2] : $_SERVER['DOCUMENT_ROOT'] ?>",
    "width":175,
    "allowBlank":false
    },
    { 	"xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('mischomeurl', true) ?>",
    "name": "home_url",
    "value": "<?= !empty($data[3]) ? $data[3] : $GLOBALS["home_url"] ?>",
    "width":175,
    "allowBlank":false
    },{
    "xtype": "combo",
    "fieldLabel": "<?= ext_Lang::msg('miscshowhidden', true) ?>",
    "store": [
    ["1", "<?= ext_Lang::msg(array('miscyesno' => 0), true) ?>"],
    ["0", "<?= ext_Lang::msg(array('miscyesno' => 1), true) ?>"]
    ],
    "hiddenName": "show_hidden",
    "disableKeyFilter": true,
    "value": "<?= (!empty($data[4]) ? $data[4] : (int) $data[4] ) ?>",
    "editable": false,
    "triggerAction": "all",
    "mode": "local",
    "allowBlank": false,
    ...
    { 	"xtype": "textfield",
    "fieldLabel": "<?= ext_Lang::msg('mischidepattern', true) ?>",
    "name": "no_access",
    "value": "<?= @$data[5] ?>",
    "width":175,
    "allowBlank":true
    ...
    {
    "xtype": "combo",
    "fieldLabel": "<?= ext_Lang::msg('miscperms', true) ?>",
    "store": [<?php
    ...
    { 	"xtype": "combo",
    "fieldLabel": "<?= ext_Lang::msg('miscactive', true) ?>",
    "store": [
    ["1", "<?= ext_Lang::msg(array('miscyesno' => 0), true) ?>"],
    ["0", "<?= ext_Lang::msg(array('miscyesno' => 1), true) ?>"]
    ],
    "hiddenName": "active",
    "disableKeyFilter": true,
    "value": "<?= (!empty($data[7]) ? $data[7] : 0 ) ?>",
    "disabled": <?= !empty($self) ? 'true' : 'false' ?>,
    "editable": false,
    "triggerAction": "all",
    "mode": "local",
    "allowBlank": false,
    ...
    "buttons": [ {
    "text": "<?= ext_Lang::msg('btnsave', true) ?>", 
    "handler": function() {
    userform = Ext.getCmp("<?= $formname ?>").getForm();
    if(userform.findField('nuser').getValue()=="" || userform.findField('home_dir').getValue()=="") {
    Ext.Msg.alert('Status', "<?= ext_Lang::err('miscfieldmissed', true); ?>");
    return false;
    }
    ...
    {
    Ext.Msg.alert('Status', "<?= ext_Lang::err('miscnopassmatch', true); ?>");
    return false;
    }
    }
    statusBarMessage( 'Please wait...', true );
    ...
    "success": function(form, action) {
    statusBarMessage( action.result.message, false, true );
    Ext.getCmp("dialog_tabpanel").remove("<?= $formname ?>");
    },
    "failure": function(form, action) {
    if( !action.result ) return;
    ...
    user: "<?= @$data[0] ?>",
    "action": 'admin', 
    "action2": "<?= @$data[0] ? 'edituser' : 'adduser' ?>",
    "confirm": "true"
    }
    })
    }
    ...
    "handler": function() { Ext.getCmp("dialog_tabpanel").remove("<?= $formname ?>"); }
    }]
    }
    <?php
}
