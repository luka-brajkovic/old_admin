    ...
    <div class="markeIndex">
        ...
        <div class="tabovi">
            <div class='tabs clear'>
                <?php
                $queryBrend = mysqli_query($conn,"SELECT id, title, logo FROM _content_brand WHERE naslovna = 'Da' AND status = 1 ORDER BY title LIMIT 6") or die(mysqli_error($conn));
                while ($data = mysqli_fetch_array($queryBrend)) {
                    ?>
                    <div class="sixths">
                        <a href='#tab<?= $data['id']; ?>' title="<?= $data['title']; ?>">
                            <img src="/uploads/uploaded_pictures/_content_brand/80x34/<?= $data['logo']; ?>" title="<?= $data['title']; ?>" alt="<?= $data['title']; ?>" />
                        </a>
                    </div>
                    <?php
                }
            ...
            <?php
            $queryBrends = mysqli_query($conn,"SELECT id, title, url, opis, logo, resource_id FROM _content_brand WHERE naslovna = 'Da' AND status = 1 ORDER BY title LIMIT 6") or die(mysqli_error($conn));
            while ($data = mysqli_fetch_array($queryBrends)) {
                ?>
                <div id='tab<?= $data['id']; ?>'>  
                    <div class="tabHolder clear">
                        <div class="quarter left">
                            <div class="tabDesc">
                                <img src="/uploads/uploaded_pictures/_content_brand/140x60/<?= $data['logo']; ?>" title="<?= $data['title']; ?>" alt="<?= $data['title']; ?>" />
                                <p><?= $data['opis']; ?></p>
                        ...
                        <div class="quarter-x3 right clear">
                            ...
                            <?php
                            $queryProizvodiBrend = mysqli_query($conn,""
                                    ...
                                    . " WHERE cp.brand = '" . $data['resource_id'] . "' AND cp.status = 1 ORDER BY cp.num_views DESC LIMIT 4") or die(mysqli_error($conn));
                            $counterBrend = 1;
                            while ($dataProizvodBrend = mysqli_fetch_array($queryProizvodiBrend)) {
                                if ($counterBrend == 1) {
                                ...
                                <div class="littleProductTab">
                                    <a class="littleProductTabAll clear" href="/<?= $dataProizvodBrend['master_cat_url'] . "/" . $dataProizvodBrend['sub_cat_url'] . "/" . $dataProizvodBrend['url'] . "/" . $dataProizvodBrend['resource_id']; ?>" title="<?= $dataProizvodBrend['title']; ?>">
                                        <figure class="third left">
                                            <img class="transition" src="/uploads/uploaded_pictures/_content_products/321x257/<?= $dataProizvodBrend['product_image']; ?>" alt="<?= $dataProizvodBrend['title']; ?>" title="<?= $dataProizvodBrend['title']; ?>" />
                                        </figure>
