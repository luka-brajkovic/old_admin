            ...
            $('#filemanagerlist').mcContextMenu({
                ...
                setup: function (m) {
                    $(m).bind('DropMenu:beforeshow', function (e, m) {
                        // Build new menu
                        m.clear();
                        m.add({title: $.translate('{#actions.cut}'), disabled: t.isDisabled('cut') || !t.selectedFiles.length, onclick: function () {
                                t.setClipBoard({files: t.selectedFiles, action: 'cut'});
                            }});
                        m.add({title: $.translate('{#actions.copy}'), disabled: t.isDisabled('copy') || !t.selectedFiles.length, onclick: function () {
                                t.setClipBoard({files: t.selectedFiles, action: 'copy'});
                            }});
                        m.add({title: $.translate('{#actions.paste}'), disabled: t.isDisabled('paste') || !t.clipboard, onclick: function () {
                                t.pasteFiles();
                            }});
                        m.addSeparator();
                        m.add({title: $.translate('{#actions.edit}'), disabled: !t.focusedFile || (!t.focusedFile.custom || !t.focusedFile.custom.editable) || t.isDisabled('edit') || !t.selectedFiles.length, onclick: function () {
                                t.editFile(t.focusedFile);
                            }});
                        m.add({title: $.translate('{#actions.rename}'), disabled: t.isDisabled('rename') || !t.selectedFiles.length, onclick: function () {
                                t.renameFile(t.focusedFile);
                            }});
                        m.add({title: $.translate('{#actions.deleteit}'), disabled: t.isDisabled('delete') || !t.selectedFiles.length, onclick: function () {
                                t.deleteFiles(t.selectedFiles);
                            }});
                        m.add({title: $.translate('{#actions.zip}'), disabled: t.isDisabled('zip') || !t.selectedFiles.length, onclick: function () {
                                t.zipFiles();
                            }});
                    });
                }
