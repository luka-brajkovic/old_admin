...
if (!$pretraga_po || strlen($pretraga_po) < 2) {
    $f->redirect("/poruka/prekratka-rec");
}
...
$descSEO = "Pretraga proizvoda na snizenju i akciji po reči $pretraga na internet prodavnici " . $configSiteFirm;
$keysSEO = "pretraga, $pretraga, " . strtolower($configSiteFirm) . ", akcije, snizenja, internet prodavnica";
...
<body>
    ...
    $query = "SELECT cp.*, b.title as b_title, c.url as master_cat_url, c1.url as sub_cat_url FROM _content_proizvodi cp "
    ...
    $executedMainQuery = mysql_query($query) or die(mysql_error());
    $num = mysql_num_rows($executedMainQuery);
    if ($num < 32) {
    ...
    $executedMainQuery = mysql_query($query . " " . $order . " " . $limits) or die(mysql_error());
    ?>
    <div class="container">
        ...
        <?php if ($num == 0) { ?>
            <h1>Trenutno nemamo proizvoda pod ovim terminom!</h1>     
            <?php
            ...
            while ($item = mysql_fetch_object($executedMainQuery)) {
