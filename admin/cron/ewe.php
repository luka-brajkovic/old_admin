...
$dom = new DOMDocument();
$dom->load("https://www.ewe.rs/share/backend_231/?user=skycomp&secretcode=322e8");
    ...
    foreach ($nodes as $n) {
        if ($n->nodeName == "products") {
            ...
            foreach ($childNodes as $product) {
                ...
                $MasterCat = mysqli_query($conn,"SELECT resource_id, procenat_dodavanje_na_cenu, title, do_50000, do_70000, do_85000, do_100000, do_120000, do_150000, do_170000, do_200000, od_200001 FROM categories WHERE title = '$category' AND parent_id = '0' LIMIT 1");
                ...
                $inBrand = new View("_content_brand", "$manufacturer", "title");
                if (empty($inBrand->id)) {
                    ...
                    if (empty($inBrand->id)) {
                        ...
                        if (empty($inBrand->id)) {
                            ...
                            $resourcesis->table_name = "_content_brand";
                            $resourcesis->Save();

                            $inBrand = new View("_content_brand");
                ...
                $trenutniBrend = new View("_content_brand", "$manufacturer", "title");
                ...
                if (empty($inItem->id)) {
                    ...
                    if ($order == "") {
                        $maxValuOrder = mysqli_query($conn,"SELECT MAX(ordering) as max_order FROM _content_products");
                    ...
                    $resourceForProduct->table_name = "_content_products";
                    $resourceForProduct->Save();

                    $newItem = new View("_content_products");
                    ...
                    $kategorijas = mysqli_query($conn,"SELECT resource_id FROM categories WHERE title = '$subcategory' AND parent_id = '$MasterCat->resource_id' LIMIT 1");
                    $kategorijaCC = mysqli_fetch_object($kategorijas);

                    $proizvodID = mysqli_query($conn,"SELECT resource_id FROM _content_products WHERE product_code = '$id' LIMIT 1") or die(mysqli_error($conn));
                    $proizvodID = mysqli_fetch_object($proizvodID);
