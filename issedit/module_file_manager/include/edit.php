...
class ext_Edit extends ext_Action {
    ...
    function execAction($dir, $item) {  // edit file
        ...
        "title": "<?= strlen($s_item) > 50 ? substr($s_item, strlen($s_item) - 30, 30) : $s_item; ?>",
        "frame": "true",
        "closable": "true",
        "tbar": [{
        "text": "<?= ext_Lang::msg('btnsave', true) ?>", 
        "handler": function() {
        statusBarMessage( '<?= ext_Lang::msg('save_processing', true) ?>', true );
        form = Ext.getCmp("<?= $id_hash ?>").getForm();
        form.submit({
        ...
        Ext.Msg.alert('<?= ext_Lang::err('error', true) ?>!', action.result.error);
        },
        scope: form,
        // add some vars to the request, similar to hidden fields
        params: {option: 'com_extplorer', 
        action: 'edit', 
        code: editAreaLoader.getValue("ext_codefield<?= $id_hash ?>"),
        ...
        "cls":"x-btn-text-icon",
        "icon": "<?= _EXT_URL ?>/images/_save.png"
        },{
        "text": "<?= ext_Lang::msg('btnreopen', true) ?>", 
        "handler": function() { 
        statusBarMessage( '<?= ext_Lang::msg('reopen_processing', true) ?>', true );
        form = Ext.getCmp("<?= $id_hash ?>").getForm();
        form.submit({
        ...
        success: function(form, action) {
        statusBarMessage( action.result.message, false, true );
        editAreaLoader.setValue("ext_codefield<?= $id_hash ?>", action.result.content);
        },
        failure: function(form, action) {
        statusBarMessage( action.result.error, false, false );
        Ext.Msg.alert('<?= ext_Lang::err('error', true) ?>!', action.result.error);
        ...
        "items": [{
        "xtype": "displayfield",
        "value": "<?= $GLOBALS["messages"]["actedit"] . ": $s_item" ?>"
        },
        {
        "xtype": "textarea",
        "hideLabel": true,
        "name": "thecode",
        "id": "ext_codefield<?= $id_hash ?>",
        ...
        "width": "<?= $cw ?>", 
        "xtype": "textfield",
        "fieldLabel": "<?= ext_Lang::msg('copyfile', true) ?>",
        "name": "fname",
        "value": "<?= $item ?>",
        "clear": "true"
        }
        <?php if ($langs == "japanese") { ?>
            ...
            "clear":"true",
            "xtype": "combo",
            "fieldLabel": "<?= ext_Lang::msg('fileencoding', true) ?>",
            "name": "file_encoding",
            "store": [
            ["UTF-8", "UTF-8"],
            ["SJIS-WIN", "SJIS"],
