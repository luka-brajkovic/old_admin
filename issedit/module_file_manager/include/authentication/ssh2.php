...
class ext_ssh2_authentication {
    ...
    function onShowLoginForm() {
        ...
        dialogtitle: "<?= ext_Lang::msg('ssh2_header') ?>",
        title: "<?= ext_Lang::msg('ssh2_login_lbl') ?>",
        frame: true,
        keys: {
        key: Ext.EventObject.ENTER,
        fn : function(){
        if (Ext.getCmp("simpleform").getForm().isValid()) {
        statusBarMessage( '<?= ext_Lang::msg('ssh2_login_check', true) ?>', true );
        Ext.getCmp("simpleform").getForm().submit({
        reset: false,
        success: function(form, action) { location.href = '<?= basename($GLOBALS['script_name']) ?>?ssh2' },
        failure: function(form, action) {
        if( !action.result ) return;
        msgbox = Ext.Msg.alert('<?= ext_Lang::err('error', true) ?>', action.result.error);
        ...
        items: [{
        xtype: "textfield",
        fieldLabel: "<?= ext_Lang::msg('username', true) ?>",
        name: "username",
        width:175,
        allowBlank:false
        },{
        xtype: "textfield",
        fieldLabel: "<?= ext_Lang::msg('password', true) ?>",
        name: "password",
        inputType: "password",
        width:175,
        allowBlank:false
        ...
        xtype: "combo",
        fieldLabel: "<?= ext_Lang::msg('ftp_hostname_port', true) ?>",
        name: "ssh2_host",
        value: "<?= extGetParam($_SESSION, 'ssh2_host') ?>",
        store: ["<?= implode('","', $GLOBALS['ext_conf']['remote_hosts_allowed']) ?>"],
        width:175,
        triggerAction: "all",
        editable: false,
        forceSelection: true,
        ...
        buttons: [{
        text: "<?= ext_Lang::msg('btnlogin', true) ?>", 
        type: "submit",
        handler: function() {
        Ext.get( 'statusBar').update( '<?= ext_Lang::msg('ftp_login_check', true) ?>' );
        Ext.getCmp("simpleform").getForm().submit({
        reset: false,
        ...
        <?php if (!ext_isXHR()) { ?>
            {
            text: '<?= ext_Lang::msg('btnreset', true) ?>', 
